...
- **Binary freshness.** Electron dev launches `noteblock-local-service/bin/noteblock-server(.exe)`
  and `noteblock-cloud-service/bin/cloud-api(.exe)`. A stale binary means debugging code that is no
  longer on disk — run `npm run dev` or `npm run build:services` first.
